...
import numpy as np
from parameters import RENDER, HORIZON, EPISODES, SEED, LOAD_MODEL, AGENT_NAME, GRID, SHOULD_LOG, LEARNING_RATE, ENTROPY_WEIGHT, SAVE_MODEL, ENV_NAME, NB_DIM, MAX_ITR_VP, BATCH_SIZE, DISCOUNT, LAMBDA, VERBOSE
from utils import deleteDir, log, update_mem_graph_dec,  prep_train_data_dec,logRunTime, get_one_hot, create_train_fg, create_train_fg_all_actions
import argparse
from environment import environment
from agents.vpg_mem import vpg_dec_mem
from agents.idv_mem import idv_dec_mem
...
def init_agent(agent_name, config):
    ...
    agent = None
    if agent_name == "vpg_dec_mem":
        agent = vpg_dec_mem(config=config)
    elif agent_name == "idv_dec_mem":
        agent = idv_dec_mem(config=config)
    else:
...
def main():
    ...
    for s in range(num_states):
        indexes = o_nbr[s][o_nbr[s] > -1]
        o_nbr_index.append(indexes)
    o_nbr_index = np.array(o_nbr_index)
    ...
    rg_tab = np.zeros((0, 1), dtype=NP_DTYPE_D)
    fg_val = np.empty((0, 1), dtype=NP_DTYPE_D)
    ...
    # ----- Memory
    x_mem = np.empty((0, num_states), dtype=NP_DTYPE_D)
    v_mem = np.empty((0, num_states, num_actions), dtype=NP_DTYPE_D)
    g_mem = np.empty((0, num_states, num_actions), dtype=NP_DTYPE_D)
    n_mem = np.empty((0, num_states, num_actions), dtype=NP_DTYPE_D)
    ...
    for ep in range(1, EPISODES + 1):
        ...
        train_x, train_val, train_nsa, fg_index = prep_train_data_dec(s_dim, o_dim, buff_ntsa, buff_x, val_fn)
        lg.writeln(" Prep TrainData Time: " + tm.logTime())

        # -------- Update Memory Graph
        x_tab_new, t_tab_new, xxp_tab_new, rg_tab, fg_val, sorted_time, fg_soa = update_mem_graph_dec(buff_at_rw, buff_ntsas, buff_ntsa, buff_nts, s_dim, a_dim, o_dim, x_tab, t_tab, xxp_tab, rg_tab, fg_val, o_nbr, val_fn, fg_soa)
        ...
        # -- Mem Graph
        # agent.plot_memGraph(xxp_tab)


        # ----------- Value Propagation
        conv_itr = np.zeros(MAX_ITR_VP)
        # fg_soa, fg_val, conv_itr = value_propagation(t_tab_new, x_tab_new, xxp_tab_new, fg_soa, fg_val, rg_tab, sorted_time, MAX_ITR_VP, fg_index, DISCOUNT, conv_itr)
        lg.writeln(" Value prop time: " + tm.logTime())

        # ---------- Create train_fg
        train_fg = np.zeros((buff_x.shape[0], num_states, num_actions))

        train_fg = create_train_fg(buff_x, x_tab_new, fg_soa, s_dim, o_dim, a_dim, num_states, num_actions, o_nbr_index, fg_val, train_fg)
        # train_fg = create_train_fg_all_actions(buff_x, x_tab_new, fg_soa, s_dim, o_dim, a_dim, num_states, num_actions, o_nbr_index, fg_val, train_fg)
        lg.writeln(" Create train_fg time: " + tm.logTime())
        ...
        if ep % SHOULD_LOG == 0:
            agent.log(ep, rt_sum)
            agent.log_agent(ep, node_len, avg_nbr)
            # agent.log_agent(ep, node_len, avg_nbr, fg_val)
            # lg.writeln(" x_node: " + str(x_tab))
            lg.writeln("\n Return: " + str(rt_sum))
