...
def value_propagation(np.ndarray[DTYPE_f, ndim=2] time_index, np.ndarray[DTYPE_f, ndim=2] nodes, np.ndarray[DTYPE_f, ndim=2] nbr, np.ndarray[DTYPE_f, ndim=2] fg_soa, np.ndarray[DTYPE_f, ndim=2] fg_val, np.ndarray[DTYPE_f, ndim=2] rg_tab, np.ndarray[DTYPE_f, ndim=1] sorted_time, DTYPE_i MAX_ITER, np.ndarray[DTYPE_f, ndim=1] fg_index, DTYPE_f Discount, np.ndarray[DTYPE_f, ndim=1] conv_itr):
	...
	cdef double[:,:] fg_soa_c = np.array(fg_soa, dtype=NP_DTYPE_F)
	cdef double[:,:] fg_soa_old = np.array(fg_soa, dtype=NP_DTYPE_F)
	cdef double[:,:] fg_val_c = np.array(fg_val, dtype=NP_DTYPE_F)
	cdef double[:,:] rg_tab_c = np.array(rg_tab, dtype=NP_DTYPE_F)
	cdef double[:] conv_itr_c = np.array(conv_itr, dtype=NP_DTYPE_F)

	# --------- indexes
	cdef int tot_nodes = nodes.shape[0]
	cdef int node_size = nodes.shape[1]
	cdef int s_max = rg_tab.shape[0]
	cdef int a_max = rg_tab.shape[1]
	cdef int fg_index_size = fg_index_int.shape[0] 
	cdef int fg_soa_size = fg_soa.shape[0] 
	...
	for i in range(MAX_ITER):
		for j in range(sorted_time_size):
			...
			if node_id != -1:			
				...
				a = nodes_int[node_id, node_size-1]
				...
				for k in range(nbr_size):
					...
					if nb_node_id != -1:
						...
						for ap in range(a_max):
							if next_action_max < fg_soa_c[nb_node_id][ap]:
								next_action_max = fg_soa_c[nb_node_id][ap]
						next_state_sum += next_action_max
						nbr_count += 1
				if nbr_count != 0:
					next_state_sum = next_state_sum / nbr_count				
					# fg_soa_c[node_id][a] = rg_tab_c[s][a] + Discount *next_state_sum
					fg_soa_c[node_id][a] = rg_tab_c[node_id][0] + Discount *next_state_sum

		# ---- check convergence
		avg_diff = 0.0
		for m in range(fg_soa_size):
			for a in range(a_max):
				diff = fg_soa_c[m][a] - fg_soa_old[m][a]
				if diff < 0:
					diff = -1 * diff
				avg_diff += diff
		avg_diff = avg_diff / (fg_soa_size*a_max)
		conv_itr_c[i] = avg_diff		

		# -- update the old
		for m in range(fg_soa_size):
			for a in range(a_max):
				fg_soa_old[m][a] = fg_soa_c[m][a]
	...
	# 	if node_id != -1:
	# 		train_fg[i] = fg_soa_c[node_id]



			
	return np.asarray(fg_soa_c), np.asarray(fg_val_c), np.asarray(conv_itr_c)
