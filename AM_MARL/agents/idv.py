...
import numba as nb
# from utils import compute_idv_thien
...
class idv_dec(baseAgent_dec):

    def __init__(self, config=None):
        super(idv_dec, self).__init__(config=config)
        ...
        if LOAD_MODEL:
            ...
            self.lg.writeln("Loading Old Model")
            self.lg.writeln("-----------------------")
            self.actor = tc.load(self.pro_folder + '/load_model'+ '/model_actor_dec_'  + self.agent_name +".pt")
            self.actor.eval()
        else:
            # Policy Network
            ip_dim = self.one_hot_dim + self.o_dim
