...
from parameters import GRID, MID_WALL, STATE_GRID_SIZE
import torch as tc
...
class Agent(WorldObj):
    def __init__(self, world, index=0, view_size=7):
        ...
        # make agents same color
        super(Agent, self).__init__(world, 'agent', world.IDX_TO_COLOR[0])
        self.pos = None
        self.dir = None
        self.index = index
...
class MultiGridEnv(gym.Env):
    ...
    def __init__(
        ...
        # ---- James
        assert GRID % STATE_GRID_SIZE == 0, "grid not multiple of state_grid"
        self.num_state_tiles = int(GRID / STATE_GRID_SIZE)

        self.nt_new = np.zeros(self.num_state_tiles*self.num_state_tiles, dtype=np.int)
        self.ntsa = np.zeros((self.num_state_tiles*self.num_state_tiles, len(actions_set.available)), dtype=np.int)
        self.ntsas = np.zeros((self.num_state_tiles*self.num_state_tiles, len(actions_set.available), self.num_state_tiles*self.num_state_tiles), dtype=np.int)

        # self.loc = np.zeros(len(agents), dtype=np.int)
        # self.loc_prev = np.zeros(len(agents), dtype=np.int)

        self.total_agents = len(agents)
        self.loc = [0 for _ in range(len(agents))]
        self.loc_prev = [0 for _ in range(len(agents))]


        self.state_space = self.num_state_tiles * self.num_state_tiles
        self.state_dim = 1
        self.action_dim = 1
        self.atomic_reward = np.zeros(self.state_space, dtype=np.int)
        ...
        if GRID == 9:
            ...
            '''
            self.obs_dim = 5
            self.obs_nbr = np.array([[0, 1, 3, -1, -1],
                                     [1, 0, 2, 4, -1],
                                     [2, 1, 5, -1, -1],
                                     [3, 0, 4, 6, -1],
                                     [4, 1, 3, 5, 7],
        ...
        # ---- James_old
        # self.nt_new = np.zeros(GRID*GRID, dtype=np.int)
        # self.ntsa = np.zeros((GRID * GRID, len(actions_set.available)), dtype=np.int)
        # self.ntsas = np.zeros((GRID * GRID, len(actions_set.available), GRID*GRID), dtype=np.int)
        # self.loc = np.zeros(len(agents), dtype=np.int)
        # self.loc_prev = np.zeros(len(agents), dtype=np.int)
        # self.one_hot = self.get_one_hot(GRID*GRID)
        # self.tile_code = np.zeros((GRID, GRID), dtype=np.int)
        # tile_id = 0

        # -------- State Tiles
        self.tile_state = np.zeros((GRID, GRID), dtype=np.int)
        bins = []
        for i in range(self.num_state_tiles+1):
            bins.append(i*STATE_GRID_SIZE)
        self.state_xy_to_state = np.zeros((self.num_state_tiles, self.num_state_tiles), dtype=np.int)
        k = 0
        for i in range(self.num_state_tiles):
            for j in range(self.num_state_tiles):
                self.state_xy_to_state[i][j] = k
                k += 1

        for i in range(GRID):
            state_x = np.digitize(i, bins)
            for j in range(GRID):
                state_y = np.digitize(j, bins)
                state_id = self.state_xy_to_state[state_x-1][state_y-1]
    ...
    def reset(self):
        ...
        for i in range(len(self.agents)):
            ...
            state_id = self.tile_state[(y, x)]
            self.nt_new[state_id] += 1
            self.loc[i] = state_id
            self.loc_prev[i] = state_id

        # nb_agents = len(self.agents)
        # nt = self.nt_new.reshape(1, GRID*GRID)
        # ns_tmp = np.tile(nt, (nb_agents, 1))
        # s_tmp = self.loc.reshape(nb_agents, 1)
        # s_tmp2 = self.one_hot[self.loc]
        # s_nt = np.concatenate((s_tmp2, ns_tmp), axis=1)
        return obs, self.nt_new, self.ntsa, self.loc
    ...
    def place_obj(self,
        ...
        if obj.type != 'goal' and MID_WALL:
            top = [0, top[1]]
            size = [int(size[0]/2), size[1]]
    ...
    def step(self, actions):
        ...
        for i in order:
            ...
            # ---- James_old
            # x, y = self.agents[i].pos
            # tile_id = self.tile_code[(y, x)]
            # self.nt_new[tile_id] += 1
            # indx = self.agents[i].index
            # self.loc[indx] = tile_id
            # prev_tile_id = self.loc_prev[indx]
            # prev_a = actions[i]
            # self.ntsas[prev_tile_id][prev_a][tile_id] += 1
            # self.loc_prev[indx] = self.loc[indx]
            ...
            self.nt_new[state_id] += 1
            indx = self.agents[i].index
            self.loc[indx] = state_id
            prev_state_id = self.loc_prev[indx]
            prev_a = actions[i]
            self.ntsa[prev_state_id][prev_a] += 1
            self.ntsas[prev_state_id][prev_a][state_id] += 1
            self.loc_prev[indx] = self.loc[indx]
            ...
            # Move forward
                ...
                if fwd_cell is not None:
                    ...
                    agent_state = self.tile_state[pos[1], pos[0]]
                    # if fwd_cell.type == 'goal':
                    if agent_state == self.goal_state:

                        self._reward(i, rewards, 1)
                        done = True
                        break
                        # self.agents[i].terminated == True
                        # del_agent_list.append(i)
                        # set_trace()
        ...
        # ---- James Delete done agents
        # for ag_ind in del_agent_list:
        #     del self.agents[ag_ind]
        #     del self.loc[ag_ind]
        #     del self.loc_prev[ag_ind]

        # print("^^^^^^^^^^ No. of agents", len(self.agents), len(self.loc), len(self.loc_prev))

        return obs, rewards, done, {}, self.nt_new.copy(), self.ntsa.copy(), self.loc, self.ntsas.copy(), goal_flag
