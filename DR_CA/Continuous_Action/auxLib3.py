...
def joinPNG(files, append="b", output="Out.png", n=3):
    ...
    t1 = files[0]
    t1 = t1.split("/")[1:-1]
    path = functools.reduce(lambda v1, v2:v1+"/"+v2, t1)
    path = "/" + path
    ...
    if append =="b":
        ...
        for item in tmp1:
            ap = "+append"
            tstr = "convert "+ap+" "
            tstr += functools.reduce(lambda v1, v2 : v1+" "+v2, item)
            tstr += " "+path+"/"+str(h1)+".png"
            os.system(tstr)
            rmv = functools.reduce(lambda v1, v2 : v1+" "+v2, item)
            os.system("rm "+rmv)
            tmp2.append(path+"/"+str(h1)+".png")
            h1 += 1
        ap = "-append"
        tstr = "convert "+ap+" "
        tstr += functools.reduce(lambda v1, v2 : v1+" "+v2, tmp2)
        tstr += " "+path+"/"+output
        os.system(tstr)
        rmv = functools.reduce(lambda v1, v2 : v1+" "+v2, tmp2)
        os.system("rm "+rmv)
        return
    elif append == "v":
        ap = "-append"
        ...
        ap = "+append"
    tstr = "convert "+ap+" "
    tstr += functools.reduce(lambda v1, v2 : v1+" "+v2, files)
    tstr += " "+output
    os.system(tstr)
    rmv = functools.reduce(lambda v1, v2 : v1+" "+v2, files)
    os.system("rm "+rmv)
