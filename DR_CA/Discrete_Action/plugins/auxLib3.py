...
class adjBar:
    ...
    x = [1, 2, 3]
    y = [[40, 20, 50],[90, 60, 50]]
    y_label = ["sdfdsf", "dsfds"]
    y_err = [[0,0,0], [0,0,0]]
    pl = adjBar(x=x)
    pl.show(y=y), y_labels=y_label, y_err=y_err)
    pl.save(y=y, y_labels=y_label, y_err=y_err)
    '''
    def __init__(self, title="", barWidth=0.2, legend_loc="upper right", xlabel="", ylabel="", x=[], yscale='linear', xlim=[],ylim=[]):
        ...
        plt.xlabel(xlabel)
        self.yscale = yscale
        plt.xticks(np.arange(len(x)), tuple(x))
        plt.gca().xaxis.grid()
        plt.ylabel(ylabel)
        if len(ylim) > 0:
    ...
    def save(self, y=[], bar_label=[], y_err=[], fname="bar.png"):
        ...
        for i in range(len(y)):
            if self.yscale == 'log':
                ...
                plt.bar(x[i], y[i], yerr=err, width=self.barWidth, label=bar_label[i], capsize=2)
            else:
                plt.bar(x[i], y[i], yerr=y_err[i], width=self.barWidth, label=bar_label[i], capsize=2)
