...
from agents.appx_dr_colby import appx_dr_colby
from agents.appx_dr_scot import appx_dr_scot
...
def init_plugin():
    ...
    global sampling_time, rollout_time, training_time, mod1, mod2, mod3, reset_st_time, tot_runtime_2, else_count, tot_runtime_list, reset_en_time, local_state
    ...
    if AGENT == "random":
        ...
        agent = appx_dr_scot(dir_name=dir_name, pro_folder=pro_folder, lg=lg)
...
def update():
    ...
    global sampling_time, rollout_time, training_time, local_state
    start = True
    step_time = now()
    ...
    if t == 0:
        ...
        route_keeper = cenv.init_env(route_keeper)
        nt, local_state = cenv.init_state()
    ...
    if AGENT == "ppo_bl":
        ...
        ntp, ntellv, rt, nm_conf, _, route_keeper, _ = cenv.step(ep=ep, t=t, state=nt, action_prob=action_prob, route_keeper=route_keeper, action_ac=action_ac, agent=AGENT)
    ...
    # elif AGENT == "mtmf":
        ...
        n_ac = len(traf.id)
        if n_ac > 0:
            action_ac = agent.get_action(state=nt, local_state=local_state)
        else:
            action_ac = None
        ...
        ntp, ntellv, rt, nm_conf, _, route_keeper, local_state = cenv.step(ep=ep, t=t, state=nt, action_prob=action_prob, route_keeper=route_keeper, action_ac=action_ac, agent=AGENT)

        agent.update_buffer(local_state)
        ...
        ntp, ntellv, rt, nm_conf, _, route_keeper, _ = cenv.step(ep=ep, t=t, state=nt, action_prob=action_prob, route_keeper=route_keeper, agent=AGENT)
    ...
    if t == HORIZON:
        ...
        if ep % SHOULD_LOG == 0:
            agent.log(ep, tot_reward, buff_act_prob, avg_tr, avg_cnf, tot_cnf, cenv.goal_reached, mean_act_count)
