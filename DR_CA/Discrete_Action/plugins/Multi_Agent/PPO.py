...
class PPO_Agent:
    ...
    def train(self, ep=None):
        ...
        for transitions in self.experience.values():
            ...
            if len(total_state) == 0:
                ...
                total_reward = np.array(discounted_rewards)
                total_A = A
        ...
        total_context = np.array(total_context)
        total_policy = np.array(total_policy)
    ...
    def log(self, ep, ep_rw, buff_act_prob, avg_tr, avg_cnf, tot_cnf, goal_reached, mean_act_count):
        ...
        num_edges = mean_act_count.shape[0]
        num_action = mean_act_count.shape[1]
