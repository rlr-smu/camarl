...
from parameters import LEARNING_RATE, CASE, SEED, ALT
import torch as tc
...
class PPO_Agent:
    def __init__(self,state_size,action_size,num_routes,numEpisodes,positions_in, positions_out,num_intruders, load_model=False, dir_name=None, actions=None, routes_list=None):
    ...
    def getRouteDistances(self):
        ...
        for i in range(len(self.positions_in)):
            wps = self.positions_in[i][:-2]
            i = 0
            dist = 0
            while i < len(wps) - 2:
                lat1 = wps[i]
        ...
        for i in range(len(self.positions_out)):
            wps = self.positions_out[i][:-2]
            i = 0
            dist = 0
            while i < len(wps) - 2:
                lat1 = wps[i]
