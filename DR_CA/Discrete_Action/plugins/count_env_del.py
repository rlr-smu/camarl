...
class countAirSim_del(syn_data):

    def __init__(self, dir_name=None, pro_folder=None):
        super(countAirSim_del,  self).__init__(dir_name=dir_name, pro_folder=pro_folder)
    ...
    def step(self, ep=None,  t=None, state=None, action_prob=None, route_keeper=None, action_ac=None, agent=None):
        ...
        if agent == "ppo":
            for i in range(len(traf.id)):
                speed = action_ac[i]
                # if speed == 1:
                #     speed = int(np.round((traf.cas[i] / tools.geo.nm) * 3600))
        ...
        # -------- Aircraft Nbrs
        self.dist_mat = np.where((0 < self.dist_mat) & (self.dist_mat < 3), -1, self.dist_mat)
        t1 = np.where(self.dist_mat == -1)
        row = t1[0]
        col = t1[1]
        self.ac_nbrs = {}
        acs = np.unique(row)
        ...
        for i in range(len(row)):
            self.ac_nbrs[row[i]].append(col[i])
        self.acs = list(self.ac_nbrs.keys())
        # print("******",self.ac_nbrs)
        # ----- Delete conflict aircraft
        # for i in self.ac_nbrs:
        #     self.delete_ac(i)
                ...
                for l2 in range(self.num_los):
                    ...
                    for v in range(self.num_actions):
                        ...
                        for id in ac_list2:
                            ...
                            #     stack.stack('{} SPD  {}'.format(idx, speed))
                            speed = self.action_space[v]
                            stack.stack('{} SPD  {}'.format(idx, speed))
    ...
    def get_distance_matrix(self):
        ...
        self.dist_mat = np.array(self.dist_mat)

        return ac_los


# class countAirSim_del_BK(syn_data):
#
#     def __init__(self, dir_name=None, pro_folder=None):
#         super(countAirSim_del,  self).__init__(dir_name=dir_name, pro_folder=pro_folder)
#
...
#                 ac_nbrs[i] = []
#             for i in range(len(row)):
#                 ac_nbrs[row[i]].append(col[i])
#             acs = list(ac_nbrs.keys())
#             for i in ac_nbrs:
#                 for j in ac_nbrs[i]:
#                     if j in acs:
#                         acs.remove(i)
...
#                         break
#
#                 # if len(ac_nbrs[i]) == 1 and ac_nbrs[i] in acs:
#                 #     acs.remove(i)
#                 #     idx = traf.id[i]
#                 #     stack.stack('DEL ' + idx)
#                 #     print(acs)
...
#         for i in range(num_ac):
#             flag = False
#             # check for delete and delete
#             if i in self.ac_nbrs:
#                 flag = self.delete_check(i)
#
#             if flag is False:
#                 x, y = traf.lat[i], traf.lon[i]
#                 e = int(self.get_edges(x, y, self.alt))
...
#
#
#         if self.abs_num_ac > self.max_ac:
#             return route_keeper
#
#         lats = self.routes_lat[rid]
#         lons = self.routes_lon[rid]
