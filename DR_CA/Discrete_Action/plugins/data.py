...
from parameters import ALT, NUM_LOS, ALPHA, BETA, FF, WIND, LOS_NBR, SEED, LARGE, HORIZON, BATCH_SIZE, MAP_ID, MAX_AC, ARR_INTERVAL, AGENT, CURR_ARR, LOCAL, MAX_WIND, HUGE, ENTROPY_WEIGHT, ACTIONS, MAX_ALT, SLICE_INDEX, NEW_NUM_AC, ARR_RATE, ARR_RATE_INTERVAL, REAL_START_TIME, INSTANCE, ENV_AC, VMAX_VMIN, DISCOUNT, REAL_DATA_PATH, DATE, REAL_END_TIME, MIN_DISTANCE, ADD_TRAFF, RANGE_STR, CSA
...
class syn_data(object):
    ...
    def __init__(self, dir_name=None, pro_folder=None, lg = None):
        ...
        if self.csa == "mid":
            self.csa_id = int((self.num_los * self.num_los * self.num_actions) / 2)

            self.csa_local = (int(self.num_los/2), int(self.num_los/2), int(self.num_actions/2))

            self.cs_id = int((self.num_los * self.num_los) / 2)


        elif self.csa == "max":
            self.csa_id = int(self.num_los * self.num_los * self.num_actions) - 1
            self.csa_local = (int(self.num_los-1), int(self.num_los - 1), int(self.num_actions-1))

            self.cs_id = int((self.num_los * self.num_los)-1)
