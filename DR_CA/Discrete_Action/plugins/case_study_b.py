...
from Multi_Agent.PPO_max import PPO_Agent_max
...
import os
from parameters import HORIZON, LOAD_MODEL, TEST_ID, EPISODES, MAP_ID, MAX_AC, ARR_INTERVAL, SEED, MAX_WIND, WIND, ACTIONS, ARR_RATE, INSTANCE, UPDATE_INTV, AGENT
...
def init_plugin():
    ...
    if ARR_RATE == -1:
        arr_rate_arrival = None
    else:
        arr_rate_arrival = loadDataStr(
        "./routes_james/" + MAP_ID+"/instance/"+str(INSTANCE)+"/arr_rate_arrival_"+str(ARR_RATE)+"_"+str(MAX_AC))
    ...
    # if "max" in AGENT:
    #     agent = PPO_Agent_max(n_states,actions_size,num_entry,EPISODES,positions,num_intruders, dir_name=dir_name, actions=ACTIONS, routes_list=routes_list)
...
def update():
    ...
    if ARR_RATE == -1:
        ...
        if ac_counter < max_ac:  ## maybe spawn a/c based on time, not based on this update interval
            for did in directions:
                if counter in arr_rate_arrival[did]:
                    i = np.random.choice(directions[did])
                    wps = positions[i][:-1]
                    hdg = positions[i][-1]
                    o_lat = wps[0]
    ...
    if not len(traf.id) == 0:
        ...
        # lg.writeln(str(counter) + "," + str(traf.id) + "," + str(round(agent.rt, 3)))
...
def reset():
    ...
    # lg.writeln(str(collisions_list))
...
def getClosestAC(state,traf,route_keeper,new_action,n_states,store_terminal,agent,last_observation,observation):
    ...
    for i in range(d.shape[0]):
        ...
        if len(closest_states) == 0:
            ...
            closest_states = np.zeros((1, agent.num_intruders, 7))
