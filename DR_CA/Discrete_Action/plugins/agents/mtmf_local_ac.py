...
class mtmf_local_ac(syn_data):

    def __init__(self, dir_name=None, pro_folder=None, lg=None):
        super(mtmf_local_ac, self).__init__(dir_name=dir_name, pro_folder=pro_folder)
        ...
        for e in range(self.num_edges):
            ...
            if len(self.edge_ac[e]) == 0:
                ...
                for i in self.edge_ac[e]:
                    ma_id = self.policy(s, i, mean_action_old, e)
    ...
    def get_action(self, t, state, mean_action, ac_edge):
        ...
        x_c = np.empty((0, self.num_los*self.num_los + self.num_edges + 1))
        ...
        for i in range(len(traf.id)):
            ...
            e = int(ac_edge[id])
            x = state[e].flatten()
            action_id = self.policy(x, id, mean_action, e)
    ...
    def get_next_state_train(self, t, state, mean_action, ac_edge):
        ...
        xp_c = np.empty((0, self.num_los * self.num_los + self.num_edges + 1))
    ...
    def train(self, ep=None):
        ...
        qp = self.Q(self.buff_xp_c)
        p_logit, _ = self.actor(self.buff_xp_a)
        pp = F.softmax(p_logit)
    ...
    def clear_buffer(self):
        ...
        self.batch_id = 0
        self.buff_x_c = np.empty((0, self.num_los*self.num_los + self.num_edges + 1))
        self.buff_xp_c = np.empty((0,self.num_los*self.num_los + self.num_edges + 1))
...
class mtmf_real(real_data):

    def __init__(self, dir_name=None, pro_folder=None, lg=None):
        super(mtmf_real, self).__init__(dir_name=dir_name, pro_folder=pro_folder)
    ...
    def policy(self, s, k, mean_action):
        ...
        action_id = np.random.choice(self.action_id_space, 1, p=prob)
        return action_id
    ...
    def get_action(self, t, state, mean_action, ac_edge):
        ...
        x_c = np.empty((0, self.num_edges*self.num_los*self.num_los + 2))
        x_a = np.empty((0, self.num_edges*self.num_los*self.num_los + 2 + 1))
        act_c = np.empty((0, 1))
        nt = state.reshape(self.num_edges*self.num_los*self.num_los)
        ac_count = 0
        for i in range(len(traf.id)):
            ...
            action = self.action_space[action_id]
            # x_a.append(np.hstack((nt, mean_action, id)))
    ...
    def get_next_state_train(self, t, state, mean_action, ac_edge):
        ...
        xp_c = np.empty((0, self.num_edges * self.num_los * self.num_los + 2))
        xp_a = np.empty((0, self.num_edges * self.num_los * self.num_los + 2 + 1))
        nt = state.reshape(self.num_edges*self.num_los*self.num_los)
        for i in range(len(traf.id)):
            id = int(traf.id[i][2:])
            e = int(ac_edge[id])
    ...
    def clear_buffer(self):
        ...
        self.data_pt = self.batch_size * self.horizon
        self.batch_id = 0
        self.buff_x_c = np.empty((0, self.num_edges*self.num_los*self.num_los + 2))
        self.buff_xp_c = np.empty((0, self.num_edges*self.num_los*self.num_los + 2))
        self.buff_x_a = np.empty((0, self.num_edges*self.num_los*self.num_los + 2 + 1))
        self.buff_xp_a = np.empty((0, self.num_edges*self.num_los*self.num_los + 2 + 1))
        self.buff_ac_id = np.zeros(self.horizon+1).astype('int32')
        self.buff_ac_act = np.empty((0, 1))
