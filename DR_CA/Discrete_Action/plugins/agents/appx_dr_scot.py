...
class appx_dr_scot(syn_data):

    def __init__(self, dir_name=None, pro_folder=None, lg=None):
        super(appx_dr_scot,  self).__init__(dir_name=dir_name, pro_folder=pro_folder)
    ...
    def store_rollouts(self, buff_nt=None, buff_ntellv=None, buff_rt=None, buff_ntev_mean=None,  buff_act_prob=None):
        ...
        # ---------
        nsa_dr = buff_ntellv.copy()
        nsa_dr = nsa_dr / self.max_ac
        nsa_r = nsa_dr.reshape(self.horizon, self.num_edges, self.num_los*self.num_los*self.num_actions)
        nsa_r = tc.tensor(nsa_r).float()
        rw = self.rt_nw(nsa_r).sum(-1)

        for l1 in range(self.num_los):
            for l2 in range(self.num_los):
                for a in range(self.num_actions):
                    d_l1, d_l2, d_a = self.csa_local
                    nsa_dr[:,:,l1,l2,a] = nsa_dr[:,:,d_l1,d_l2,d_a]

        nsa_dr = nsa_dr.reshape(self.horizon, self.num_edges, self.num_los*self.num_los*self.num_actions)

        nsa_dr = tc.tensor(nsa_dr).float()
        rw_cf = self.rt_nw(nsa_dr)
        rw_cf = rw_cf.sum(-1)


        dr = (rw - rw_cf).data.numpy()

        for e in range(self.num_edges):

            self.buff_diff_return[e] = discounted_return(dr[:, e])
        # -------- update batch id
        self.batch_id += 1
    ...
    def train(self, ep=None):
        ...
        for i in range(EPOCH):
            ...
            op3 = tc.tensor(op3.reshape(self.horizon, self.num_edges, self.num_los*self.num_los))
            op4 = op1 * op3
            op4 = op4.sum(-1).sum(-1)
            op5 = op4 * op4
            loss = op5.sum()
            ...
            # dtpt = y_target.shape[0] * y_target.shape[1] * y_target.shape[2]
        ...
        if ep > POLICY_TRAIN:
            for e in range(self.num_edges):
                ...
                gt = tc.tensor(self.buff_diff_return[e]).float()
                op2 = tc.mul(op1, gt)
...
class appx_dr_scot_real(real_real_data):

    def __init__(self, dir_name=None, pro_folder=None, lg=None):
        super(appx_dr_scot_real,  self).__init__(dir_name=dir_name, pro_folder=pro_folder)
