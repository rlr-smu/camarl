...
from parameters import LOAD_MODEL, LEARNING_RATE, DISCOUNT, NUM_CORES, PPO_EPOCH, PPO_CLIP, ENTROPY_WEIGHT
...
class diff_ppo_max(syn_data):

    def __init__(self, dir_name=None, pro_folder=None, lg=None):
        super(diff_ppo_max,  self).__init__(dir_name=dir_name, pro_folder=pro_folder)
        ...
        if LOAD_MODEL:
            ...
            for e in range(self.num_edges):
                ...
                self.network_list_old.append(network(ip_dim, self.num_actions))

                self.network_list_old[e].load_state_dict(self.network_list[e].state_dict())
    ...
    def get_action(self, state=None):
        ...
        with tc.no_grad():
            for e in range(self.num_edges):
                pi_logit, _ = self.network_list_old[e](state[e].flatten())
    ...
    def train(self, ep=None):
        ...
        tmp_data['pred'] = y_pred.data.numpy()
        tmp_data['target'] = y_target.data.numpy()
        ...
        for e in range(self.num_edges):
            for _ in range(PPO_EPOCH):
                ...
                # ------ Fwd Pass
                log_pi_old = tc.FloatTensor(self.buff_act_prob[:,e,:])
                ...
                ratios = tc.exp(log_pi - log_pi_old.detach())

                ntev_e = self.buff_ntellv[:,e,:,:,:]
                ntev = tc.tensor(ntev_e.sum(1).sum(1)).float()

                diff_gt = tc.tensor(self.buff_diff_return[e]).float().unsqueeze(dim=1)

                adv = diff_gt * ntev

                surr1 = ratios * adv
                surr1 = surr1.sum(1).mean()
                surr2 = tc.clamp(ratios, 1-PPO_CLIP, 1+PPO_CLIP) * adv
                surr2 = surr2.sum(1).mean()

                pi_loss = -tc.min(surr1, surr2) - ENTROPY_WEIGHT*entropy
        ...
        # pred = pred_plot(tmp_data=tmp_data, ppath=self.pro_folder+"/log/"+self.dir_name+"/pred.png")
        # self.writer.add_figure("Prediction", pred.fig, ep)
