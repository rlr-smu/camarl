...
from parameters import LOAD_MODEL, LEARNING_RATE, DISCOUNT, NUM_CORES
...
class diff_rw_ana(syn_data):

    def __init__(self, dir_name=None, pro_folder=None, lg=None):
        super(diff_rw_ana,  self).__init__(dir_name=dir_name, pro_folder=pro_folder)
        ...
        self.at_rw_mat = np.tile(self.at_rw, (self.num_edges, self.num_los, 1))

        self.at_rw_mat = self.at_rw_mat.reshape(1, self.num_edges, self.num_los, self.num_los, 1)
    ...
    def store_rollouts(self, buff_nt=None, buff_ntellv=None, buff_rt=None, buff_ntev_mean=None):
        ...
        input = tc.FloatTensor(buff_ntellv)
        input.requires_grad = True
        at_rw_mat = tc.FloatTensor(self.at_rw_mat)
        op1 = input * at_rw_mat
        rt = op1.sum()
        grad = tc.autograd.grad(rt, input)[0]
        grad = grad/self.max_ac

        grad = grad.reshape(grad.shape[0], self.num_edges, self.num_los*self.num_los*self.num_actions)
